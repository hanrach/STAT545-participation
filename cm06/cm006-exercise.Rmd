...
Just creates a new tibble with `cont` column.

```{r}
rename(gapminder, cont= continent)
```

Actually renames the column variable name. 
...
```{r}
arrange(gapminder, year, desc(year))
```
...
```{r}
gapminder %>%
  mutate(gdpBill = gdpPercap*pop/(10**9))
```
...
```{r}
gapminder %>%
  transmute(country,gdpBill = round(gdpPercap*pop/(10^9),2))
```
...
```{r}
# gapminder %>%
#   mutate(lifeExp=if_else(country=="Japan" & year == 1952, 70, lifeExp))

gapminder %>%
  mutate(lifeExp=if_else(country=='Japan' &
                           year==1952, 100, lifeExp)) %>%
  filter(country == "Japan", year ==1952)
```
