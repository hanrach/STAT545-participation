...
gapminder %>% 
  ...
  geom_point(shape=21,alpha = 0.4) + scale_size_area()+
...
But does horizontal bar graph make sense?
...
gapminder %>% 
  ...
  geom_density_ridges() +
  geom_rug(aes(group=country))
```

What's wrong with the bar code looking thing at the bottom??
...
ggplot(hair_eye, aes(Hair, Eye)) +
  geom_tile(aes(fill = n),show.legend = TRUE)
```
