...
gapminder %>% 
  group_by(country) %>% 
  ggplot(aes(year, lifeExp)) + geom_line(aes(group=country, color=country), alpha = 0.5) + theme(legend.position = "none")
```
...
gapminder %>% 
  ...
  geom_point(shape=21,alpha = 0.2) + scale_size_area()+
...
tribble(
  ...
  geom_path(arrow = arrow()) + geom_text(aes(label=time))
```
