...
1. A car weighs 4000 lbs. What can we say about its mpg? --> Relatively low.
2. A car weights less than 3000 lbs. What can we say about its mpg? --> Relatively high.
...
Nothing much can be concluded from the plot below.
...
```{r}
years1 = data.frame(year = c(2020,2030,2050))
...
```{r}
#y <- lifeExp object lifeExp not found
lm_linear <- lm(data = gapminder,
                formula = lifeExp~poly(I(year-1952), degree=1))
...
(lm_extract_stat <- tidy(lm_cat2))

```

```{r}
(lm_augment <- augment(lm_cat2))
```
```{r}
(lm_glance <- glance(lm_cat))
```
